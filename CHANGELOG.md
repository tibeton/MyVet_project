...
кнопкой — на экране не менялось ничего. Новый `components/ShowMore.tsx` в
`useLayoutEffect` сдвигает прокрутку ровно на столько, на сколько уехала
кнопка: она остаётся под курсором, а строки появляются прямо над ней.

Кнопка общая для прайса и FAQ, вид не изменился.
...
### Changed — липкая колонка в «Запишите питомца на приём»
Карточка формы залипает на `lg:top-28`, как левая колонка в прайсе; сетка
получила `lg:items-start` — без него колонка растягивается на всю высоту
строки и залипать ей негде.
